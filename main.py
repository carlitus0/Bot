...
# =========================
# DYNOLIKE SERVICE EXTENSION (SAFE ADD-ON)
# =========================
...
automod_enabled = {}
automod_words = {}  # guild_id -> set(words)
automod_action = {}  # guild_id -> "delete" | "warn"
automod_state = True


def automod_is_bad(message: str, words: set):
    msg = message.lower()
...
async def on_message(message):
    ...
    if guild:
        ...
        if automod_enabled.get(gid, False) and automod_state:

            words = automod_words.get(gid, set())

            if words and automod_is_bad(message.content, words):

                action = automod_action.get(gid, "delete")
                    ...
                    if action == "delete":
                        ...
                        try:
                            await message.channel.send(
                                f"⚠️ {message.author.mention} mensagem bloqueada pelo automod."
                            )
                        except:
                            pass
                except:
...
async def automod_add(ctx, *, word: str):
    ...
    if gid not in automod_words:
        automod_words[gid] = set()

    automod_words[gid].add(word.lower())

    await ctx.send(f"✔ Palavra adicionada no automod: `{word}`")


@bot.command()
@commands.has_permissions(administrator=True)
async def automod_remove(ctx, *, word: str):
    gid = ctx.guild.id

    if gid in automod_words:
        automod_words[gid].discard(word.lower())
...
async def automod_on(ctx):
    automod_enabled[ctx.guild.id] = True
    await ctx.send("✔ Automod ativado")


@bot.command()
@commands.has_permissions(administrator=True)
async def automod_off(ctx):
    automod_enabled[ctx.guild.id] = False
    await ctx.send("❌ Automod desativado")


@bot.command()
@commands.has_permissions(administrator=True)
async def automod_action_set(ctx, action: str):
    if action not in ["delete", "warn"]:
        return await ctx.send("Use: delete ou warn")

    automod_action[ctx.guild.id] = action
    await ctx.send(f"✔ Ação do automod definida: `{action}`")


@bot.command()
@commands.has_permissions(administrator=True)
async def automod_clear(ctx):
    automod_words[ctx.guild.id] = set()
    await ctx.send("✔ Lista de automod limpa")


# =========================
# SERVER STATUS (BÁSICO)
# =========================
...
class ControlPanel(discord.ui.View):
    ...
    @discord.ui.button(label="Automod ON", style=discord.ButtonStyle.success)
    async def on_btn(self, interaction, button):
        automod_enabled[interaction.guild.id] = True
        await interaction.response.send_message("Automod ligado", ephemeral=True)

    @discord.ui.button(label="Automod OFF", style=discord.ButtonStyle.danger)
    async def off_btn(self, interaction, button):
        automod_enabled[interaction.guild.id] = False
        await interaction.response.send_message("Automod desligado", ephemeral=True)

    @discord.ui.button(label="Clear Words", style=discord.ButtonStyle.secondary)
    async def clear_btn(self, interaction, button):
        automod_words[interaction.guild.id] = set()
        await interaction.response.send_message("Lista limpa", ephemeral=True)


@bot.command()
async def control(ctx):
    await ctx.send("📌 Painel de controle:", view=ControlPanel())


# =========================
# STOP GLOBAL (KILL SWITCH)
# =========================

@bot.command()
@commands.has_permissions(administrator=True)
async def automod_stopall(ctx):
    automod_enabled.clear()
    automod_words.clear()
    automod_action.clear()
    await ctx.send("🛑 Automod global desligado")
